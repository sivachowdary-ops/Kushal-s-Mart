...
export function ProductCard({
  ...
  return (
    <div className="group relative flex flex-col justify-between rounded-2xl sm:rounded-[28px] border border-gray-100 bg-white p-3 sm:p-5 shadow-xs transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl">
      <div>
        {/* Top Badges Row */}
        ...
        {/* Product Image Area */}
        <Link href={`/product/${slug}`} className="block overflow-hidden py-2 sm:py-4 text-center">
          {displayImage ? (
            <img
              src={displayImage}
              alt={name}
              className="mx-auto h-28 sm:h-44 w-full object-contain transition-transform duration-300 group-hover:scale-105"
              loading="lazy"
            />
          ) : (
        ...
        {/* Title */}
        <Link href={`/product/${slug}`} className="block mt-1 sm:mt-2">
          <h3 className="min-h-[2rem] sm:min-h-[2.5rem] text-xs sm:text-sm font-semibold leading-snug text-gray-900 transition-colors group-hover:text-red-600 line-clamp-2">
            {name}
          </h3>
        </Link>
      ...
      <div className="mt-2 sm:mt-4">
        ...
        <div className="flex flex-wrap items-baseline gap-1 sm:gap-2 mb-2 sm:mb-4">
          <span className="font-extrabold text-base sm:text-2xl text-gray-900">
        ...
        <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
          <button
            ...
            className={`flex items-center justify-center gap-1 rounded-xl sm:rounded-2xl py-2 sm:py-3 text-[10px] sm:text-xs font-bold uppercase tracking-wider text-white shadow-xs transition-all cursor-pointer ${
              isAdded ? "bg-emerald-600 scale-95" : "bg-black hover:bg-red-600 active:scale-95"
            }`}
          >
            {isAdded ? (
              <>
          ...
          <Link
            href={`/product/${slug}`}
            className="flex items-center justify-center gap-1 rounded-xl sm:rounded-2xl bg-[#111625] py-2 sm:py-3 text-[10px] sm:text-xs font-bold uppercase tracking-wider text-white shadow-xs transition-colors hover:bg-red-600"
          >
            <span>Details</span>
