...
import { useAdminStore, AdminVariant } from "@/lib/admin-store";
...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = use(params);
  const { products, categories, updateProduct, isLoading, refreshProducts } = useAdminStore();
  ...
  useEffect(() => {
    refreshProducts().then(() => setIsInitializing(false));
  }, [refreshProducts]);
  ...
  useEffect(() => {
    if (!isInitializing) {
      ...
      if (product) {
        ...
        setMrp(product.mrp ? (product.mrp / 100).toString() : "");
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${isActive ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-amber-50 text-amber-700 border border-amber-200'}`}>
                {isActive ? "Active" : "Draft"}
              </span>
            </div>
            <p className="text-xs text-slate-500 hidden sm:block">Edit product details, variant photos, and stock</p>
          </div>
        </div>
      ...
      <form id="product-edit-form" onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input required type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-lg border border-slate-300 p-2.5 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Slug *</label>
                  <input required type="text" value={slug} onChange={(e) => setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]+/g, "-").replace(/(^-|-$)+/g, "").replace(/-{2,}/g, "-"))} className="w-full rounded-lg border border-slate-300 p-2.5 focus:border-blue-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Description</label>
                  <textarea rows={4} value={description} onChange={(e) => setDescription(e.target.value)} className="w-full rounded-lg border border-slate-300 p-2.5 focus:border-blue-500" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Category</label>
                    <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="w-full rounded-lg border border-slate-300 p-2.5 focus:border-blue-500">
                  ...
                  <div>
                    ...
                    <input type="text" value={brand} onChange={(e) => setBrand(e.target.value)} className="w-full rounded-lg border border-slate-300 p-2.5 focus:border-blue-500" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input required type="number" min="0" step="0.01" value={sellingPrice} onChange={(e) => setSellingPrice(e.target.value)} className="w-full rounded-lg border border-slate-300 p-2.5 focus:border-blue-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">MRP</label>
                  <input type="number" min="0" step="0.01" value={mrp} onChange={(e) => setMrp(e.target.value)} className="w-full rounded-lg border border-slate-300 p-2.5 focus:border-blue-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Cost Price</label>
                  <input type="number" min="0" step="0.01" value={costPrice} onChange={(e) => setCostPrice(e.target.value)} className="w-full rounded-lg border border-slate-300 p-2.5 focus:border-blue-500" />
                </div>
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">Manage color/model variants, individual stock counts, and price overrides.</p>
                </div>
                <button
                  type="button"
                  onClick={() => setVariants([...variants, { name: "", sku: "", stock: 0, selling_price_override: null, mrp_override: null, low_stock_threshold: 3, images: [] }])}
              ...
              <div className="space-y-4">
                ...
                <div className="pt-4 border-t border-slate-200 flex items-center justify-between bg-slate-50/60 -mx-6 -mb-6 p-4 rounded-b-3xl">
                  <span className="text-xs text-slate-500 font-medium">
                    Finished picking photos for variants? Save your progress:
                  </span>
                  <button
                    type="submit"
                    form="product-edit-form"
                  ...
                  >
                    ...
                    <span>{isSubmitting ? "Saving..." : isSaved ? "Saved!" : "Save Variants & Photos"}</span>
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="sticky bottom-4 z-20 flex items-center justify-between rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-xl backdrop-blur-md">
          <div className="flex items-center gap-2">
            {isSaved ? (
              ...
              <span className="text-xs text-slate-500 font-medium">
                Remember to click Save Changes to persist photo assignments.
              </span>
            )}
          </div>
          <div className="flex items-center gap-3">
